package main_project_025.I6E1.global.aws;
...
public class AwsS3Service {
    ...
    @Value(("${cloud.aws.resize.bucket.address}"))
    private String resizeAddress;
    ...
    public List<String> uploadImage(List<MultipartFile> multipartFile) {//multipartFile 인터페이스 사용해 업로드
        ...
        multipartFile.forEach(file -> {//forEach 메소드를 사용해 multipartFile에 담긴 파일을 하나씩 처리
            ...
            String urlPath = new StringBuilder(bucketAddress).append(fileName).toString();
    ...
    public List<String> uploadThumbnail(List<MultipartFile> multipartFile) {//multipartFile 인터페이스 사용해 업로드
        ...
        multipartFile.forEach(file -> {//forEach 메소드를 사용해 multipartFile에 담긴 파일을 하나씩 처리
            ...
            String urlPath = new StringBuilder(resizeAddress).append(fileName).toString();
            String urlPath2 = new StringBuilder(bucketAddress).append(fileName).toString();
            fileUrlList.add(urlPath);
